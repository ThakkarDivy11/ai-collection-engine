...
exports.getDetailedRiskAnalysis = async (invoiceData, suggestedTone) => {
    try {
        ...
        - Use escaped newlines (\\n) for any newlines in strings. Do not use actual newlines inside string values.`;
        ...
        if (jsonMatch) {
            ...
            try {
                ...
                // Then try to fix unescaped newlines inside strings
                // This is a common issue with LLMs returning "pretty" JSON with raw newlines in values
                const sanitized = jsonString
                    .replace(/[\x00-\x08\x0B-\x0C\x0E-\x1F\x7F-\x9F]/g, "")
                    .replace(/:\s*"([^"]*)"/g, (match, p1) => {
                        // Replace actual newlines inside quotes with escaped \n
                        return `: "${p1.replace(/\n/g, "\\n").replace(/\r/g, "\\r")}"`;
                    });
                
                return JSON.parse(sanitized);
            } catch (e) {
                console.error("JSON Parse failed. Original content:", jsonString);
                // Last ditch effort: remove ALL newlines and hope for the best
                try {
                    const superClean = jsonString.replace(/\n/g, " ").replace(/\r/g, " ");
