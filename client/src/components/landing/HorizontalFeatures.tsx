...
const HorizontalFeatures: React.FC = () => {
  ...
  const features = [
    {
      ...
      icon: <BrainCircuit size={48} className="text-calypso-500 dark:text-calypso-300" />,
      stat: "99.2% Accuracy",
      color: "from-calypso-500/5 dark:from-calypso-300/5",
      mockup: {
    ...
    },
    {
      ...
      icon: <Users size={48} className="text-calypso-500 dark:text-calypso-300" />,
      stat: "4.2x Recovery Rate",
      color: "from-calypso-400/5 dark:from-calypso-200/5",
      mockup: {
        title: "Active Negotiations",
        subtitle: "Neural NLP Engine",
        rows: [
          { label: "Settlement Found", value: "₹45,000", color: "bg-calypso-500" },
          { label: "Agent Response", value: "Pending", color: "bg-calypso-500" },
          { label: "User Sentiment", value: "Positive", color: "bg-calypso-500" }
        ]
      }
    },
    {
      ...
      icon: <Zap size={48} className="text-calypso-500 dark:text-calypso-300" />,
      stat: "0ms Latency",
      color: "from-calypso-600/5 dark:from-calypso-400/5",
      mockup: {
        title: "Operation Pipeline",
        subtitle: "Autonomous Workflows",
        rows: [
          { label: "Auto-Trigger", value: "Active", color: "bg-calypso-500" },
          { label: "Batch Process", value: "1.2s", color: "bg-calypso-500" },
          { label: "Queue Status", value: "Optimal", color: "bg-calypso-500" }
        ]
      }
    },
    {
      ...
      icon: <Search size={48} className="text-calypso-500 dark:text-calypso-300" />,
      stat: "₹250Cr+ Analyzed",
      color: "from-calypso-700/5 dark:from-calypso-500/5",
      mockup: {
        title: "Forensic Analysis",
        subtitle: "Structural Leak Detection",
        rows: [
          { label: "Leak Detected", value: "₹2.4M", color: "bg-rose-500" },
          { label: "Pattern ID", value: "#XJ-92", color: "bg-calypso-500" },
  ...
  return (
    <div className="relative">
      {/* Mobile Stacked View */}
      <div className="md:hidden space-y-10 px-6 py-20 bg-calypso-50 dark:bg-[#1a2a37]">
         <div className="text-center mb-16">
            <span className="text-calypso-600 dark:text-calypso-300 font-semibold text-xs uppercase tracking-[0.4em] mb-4 block">Core Engine</span>
            <h2 className="text-4xl font-semibold text-slate-900 dark:text-white tracking-tighter">Sophisticated Recovery.</h2>
         </div>
         {features.map((f, i) => (
            <div key={i} className="p-8 rounded-[2rem] bg-white dark:bg-white/[0.02] border border-calypso-100 dark:border-calypso-800/20">
                ...
                <div className="text-calypso-600 dark:text-calypso-300 font-bold text-sm tracking-widest uppercase">{f.stat}</div>
            </div>
         ))}
      </div>

      {/* Desktop Horizontal View */}
      <div ref={triggerRef} className="hidden md:block overflow-hidden bg-calypso-50 dark:bg-[#1a2a37]">
            ...
            <div key={i} className={`w-screen h-screen flex items-center justify-center relative px-20 overflow-hidden`}>
                ...
                <div className="max-w-7xl w-full grid grid-cols-2 gap-20 items-center relative z-10 text-left">
                    <div className="space-y-10">
                        <MotionWrapper>
                             <div className="inline-flex items-center gap-3 px-4 py-1.5 rounded-full bg-white dark:bg-white/5 border border-calypso-100 dark:border-calypso-800/30 text-calypso-700 dark:text-calypso-300 text-[10px] font-semibold tracking-widest uppercase mb-4">
                            ...
                            {f.title.split(' ').map((word, idx) => (
                                <React.Fragment key={idx}>
                                    {word === 'Negotiators' || word === 'Prediction' || word === 'Forensic' || word === 'Operations' ? (
                                        <span className="text-calypso-500 dark:text-calypso-300">{word}</span>
                                    ) : word}
                                    {' '}
                                </React.Fragment>
                            ))}
                        ...
                        <div className="flex items-center gap-10">
                            ...
                            <button className="flex items-center gap-3 text-slate-900 dark:text-white font-semibold group border-b border-calypso-200 dark:border-calypso-800/30 pb-2 hover:border-calypso-500 transition-all">
                                Explore Tech <ChevronRight size={18} className="group-hover:translate-x-1 transition-transform" />
                            </button>
                        </div>
                    </div>

                    <div className="relative aspect-[4/3] rounded-[3rem] bg-white dark:bg-white/[0.03] border border-calypso-100 dark:border-calypso-800/20 overflow-hidden shadow-xl group">
                         {/* Mock UI Preview */}
                         <div className="absolute inset-10 flex flex-col justify-between">
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    ...
                                    <div className="text-[10px] font-bold text-slate-400 dark:text-calypso-500 uppercase tracking-widest">{f.mockup.subtitle}</div>
                                </div>
                                <div className="w-12 h-12 rounded-2xl bg-calypso-500/10 flex items-center justify-center">
                                    {f.icon}
                                </div>
                            </div>
                            ...
                            <div className="space-y-4">
                                {f.mockup.rows.map((row, idx) => (
                                    <div key={idx} className="h-14 w-full bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-calypso-800/20 rounded-2xl flex items-center px-6 justify-between group-hover:bg-slate-100/50 dark:group-hover:bg-white/10 transition-colors shadow-sm">
