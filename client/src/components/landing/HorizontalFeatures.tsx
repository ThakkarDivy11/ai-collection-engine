...
const HorizontalFeatures: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      const pin = gsap.to(horizontalSection, {
        x: -scrollDistance,
        ease: "none",
        scrollTrigger: {
            ...
            <div key={i} className="w-screen h-screen flex items-center justify-center relative px-6 md:px-20 overflow-hidden">
                ...
                <div className="max-w-7xl w-full grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center relative z-10 text-left">
                    <div className="space-y-6 md:space-y-10">
                        ...
                        <h2 className="text-4xl sm:text-5xl md:text-[7rem] font-semibold text-slate-900 dark:text-white tracking-tighter leading-[1.1] md:leading-[0.9]">
                        ...
                        <p className="text-lg md:text-2xl text-slate-600 dark:text-slate-400 max-w-xl leading-relaxed font-normal">
                            {f.desc}
                        </p>
                        
                        <div className="flex flex-wrap items-center gap-6 md:gap-10">
                            <div>
                                <div className="text-2xl md:text-4xl font-semibold text-slate-900 dark:text-white mb-1">{f.stat}</div>
                    ...
                    <div className="relative aspect-video md:aspect-[4/3] rounded-[2rem] md:rounded-[3rem] bg-white dark:bg-white/[0.03] border border-matisse-100 dark:border-matisse-800/20 overflow-hidden shadow-xl group">
                         {/* Mock UI Preview */}
                         <div className="absolute inset-6 md:inset-10 flex flex-col justify-between">
                            <div className="flex justify-between items-start">
                                <div className="space-y-1">
                                    <div className="text-xs md:text-sm font-bold text-slate-900 dark:text-white tracking-tight">{f.mockup.title}</div>
                                    <div className="text-[9px] md:text-[10px] font-bold text-slate-400 dark:text-matisse-500 uppercase tracking-widest">{f.mockup.subtitle}</div>
                                </div>
                                <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-matisse-500/10 flex items-center justify-center scale-75 md:scale-100">
                                    {f.icon}
                                </div>
                            </div>
                            ...
                            <div className="space-y-2 md:space-y-4">
                                {f.mockup.rows.map((row, idx) => (
                                    <div key={idx} className="h-10 md:h-14 w-full bg-slate-50/50 dark:bg-white/5 border border-slate-100 dark:border-matisse-800/20 rounded-xl md:rounded-2xl flex items-center px-4 md:px-6 justify-between group-hover:bg-slate-100/50 dark:group-hover:bg-white/10 transition-colors shadow-sm">
                                        <div className="flex gap-2 md:gap-4 items-center">
                                            <div className={`w-1.5 h-1.5 md:w-2 md:h-2 rounded-full ${row.color}`} />
                                            <div className="text-[10px] md:text-sm font-semibold text-slate-600 dark:text-slate-300">{row.label}</div>
                                        </div>
                                        <div className="text-[10px] md:text-xs font-bold text-slate-900 dark:text-white uppercase tracking-wider">{row.value}</div>
                                    </div>
                                ))}
                            </div>
                         </div>
